...
rules: [/^(?:\s+)/,/^(?:[0-9]+\.[0-9]+\b)/,/^(?:[0-9]+\b)/,/^(?:(true|false))/,/^(?:and\b)/,/^(?:"(\\["]|[^"])*")/,/^(?:\$\{[a-zA-Z][a-zA-Z0-9_]*\})/,/^(?:FROM\b)/,/^(?:WHERE\b)/,/^(?:DOC\b)/,/^(?:COLLECTION\b)/,/^(?:COLGROUP\b)/,/^(?:ORDER\b)/,/^(?:BY\b)/,/^(?:LIMIT\b)/,/^(?:ASC\b)/,/^(?:DESC\b)/,/^(?:\[)/,/^(?:\])/,/^(?:,)/,/^(?:<=)/,/^(?:<)/,/^(?:==)/,/^(?:>=)/,/^(?:>)/,/^(?:!=)/,/^(?:array-contains-any\b)/,/^(?:array-contains\b)/,/^(?:in\b)/,/^(?:not-in\b)/,/^(?:[a-zA-Z]{1,}[a-zA-Z0-9_]*)/,/^(?:$)/,/^(?:.)/],
